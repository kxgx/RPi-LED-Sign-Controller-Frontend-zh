...
export default function PlaylistView({ onEditItem, onAddNewItem }: PlaylistViewProps) {
  ...
  const handleRemoveItem = async (id: string) => {
    try {
      ...
      setStatus({ message: '条目已成功删除', type: 'success' });
    } catch (error) {
      setStatus({
        message: `删除条目失败: ${error instanceof Error ? error.message : '未知错误'}`,
        type: 'error'
      });
    }
  };
  ...
  const handleMoveUp = async (index: number) => {
    ...
    try {
      ...
      setStatus({
        message: `更新顺序失败: ${error instanceof Error ? error.message : '未知错误'}`,
        type: 'error'
      });
  ...
  const handleBrightnessChange = async (value: number) => {
    ...
    try {
      ...
      setStatus({
        message: `更新亮度失败: ${error instanceof Error ? error.message : '未知错误'}`,
        type: 'error'
      });
    }
  };
